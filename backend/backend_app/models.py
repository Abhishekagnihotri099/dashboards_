...
class Claims(models.Model):
    ...
    def payout_amount(self):
        X = self.paid_amount or 0
        Y = self.remaining_reserve or 0
        Z = self.total_recovery or 0
        return X + Y - Z
...
class dsoutcome(models.Model):
    ...
    # Abhishek/Changed/commented due to signal_generated_not found in model so replaceing it with created_date
    @property
    def signal_close_date_(self):
        comparison_date = datetime(2023, 1, 1)
        
        if (isinstance(self.signal_close_date, datetime) and 
            self.signal_close_date < comparison_date):
            return self.created_date + timedelta(days=3)
...
def calculate_and_store_values(sender, instance, **kwargs):
    ...
    instance.stored_exclusions = instance.exclusions
    instance.stored_opp_identified = instance.opp_identified
    instance.stored_opp_identified_ratio = instance.opp_identified_ratio
