...
def filter_compliance_review(request):
    ...
    #     query &= Q(claim__general_nature_of_loss__in=filters['nature_of_loss'])
    # if filters['cause_of_loss']:
    #     query &= Q(claim__loss_claim_cause__in=filters['cause_of_loss'])
    # if filters['claim_status']:
    #     query &= Q(claim__claim_status__in=filters['claim_status'])
    ...
    # Query database
    signals = dsoutcome.objects.filter(query).select_related('claim__audits')
    ...
    table_data = signals.values('parameter__parameter_id', 'claim__audits__audit_date', 'stored_leakage_category', 'consultant_name', 'ai_signal', 'stored_ai_error_final', 'notification_comment', 'parameter_name').annotate(
        claims_monitored=Count('claim_id'),
        claims_with_compliance_opportunity=Count('claim_id', filter=Q(ai_signal='Yes') & Q(signal_id__in=["S7", "S8", "S9", "S10", "S11"])),
        compliance_opportunity_percentage=Case(
            When(claims_monitored__gt=0, then=F('claims_with_compliance_opportunity') * 100.0 / F('claims_monitored')),
    ...
    fig1 = go.Figure()
    fig1.add_trace(go.Bar(x=df_table['claim__audits__audit_date'], y=df_table['claims_with_compliance_opportunity'], name='Claims with Compliance Opportunity'))
    fig1.add_trace(go.Scatter(x=df_table['claim__audits__audit_date'], y=df_table['compliance_opportunity_percentage'], mode='lines', name='Compliance Opportunity %'))
    fig1.add_trace(go.Scatter(x=df_table['claim__audits__audit_date'], y=df_table['claims_monitored'], mode='lines', name='Claims Monitored'))
    fig1.update_layout(title='Compliance Opportunities Trend', xaxis_title='Audit Date', yaxis_title='Count')
    graphs['compliance_opportunities_trend'] = json.loads(fig1.to_json())
    
    # 2nd Graph: Compliance Opportunities by Category (Pie Chart)
    df_pie = df_table.groupby('stored_leakage_category').sum(numeric_only=True).reset_index()
    fig2 = px.pie(df_pie, names='stored_leakage_category', values='claims_with_compliance_opportunity', title='Compliance Opportunities by Category')
    graphs['compliance_opportunities_by_category'] = json.loads(fig2.to_json())
    
    # 3rd Graph: Compliance Opportunities by Claim Consultant (Treemap)
    df_treemap = df_table.groupby('consultant_name').sum(numeric_only=True).reset_index()
    ...
    graphs['compliance_opportunities_by_consultant'] = json.loads(fig3.to_json())
    ...
    graphs['compliance_opportunities_by_parameter'] = json.loads(fig4.to_json())
    ...
    fig5.add_trace(go.Scatter(x=df_table['consultant_name'], y=df_table['stored_ai_error_final'], mode='lines', name='AI Error Final'))
    fig5.update_layout(title='Error % by Claim Consultant', xaxis_title='Consultant Name', yaxis_title='AI Signal')
    graphs['error_by_consultant'] = json.loads(fig5.to_json())
    
    # 6th Graph: Claim Opportunity by Compliance Reasons and Category (Ribbon Graph)
    df_sunburst = df_table.dropna(subset=['notification_comment', 'stored_leakage_category'])
    fig6 = px.sunburst(df_sunburst, path=['notification_comment', 'stored_leakage_category'], values='claims_with_compliance_opportunity', title='Claim Opportunity by Compliance Reasons and Category')
    graphs['claim_opportunity_by_compliance_reasons'] = json.loads(fig6.to_json())
